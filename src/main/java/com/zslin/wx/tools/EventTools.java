...
//		System.out.println("====config:"+config);
	...
	public boolean eventRemind(String toUser,
		...
		return sendMsg(toUser, wxConfigTools.getWxConfig().getEventTemp(), url, "#FF0000", paramList);
	}
	 ...
	 * @return
	 */
	public String getTempQr(String sceneStr) {
		return getQr(1860l, "QR_STR_SCENE", sceneStr);
	}

	/**
	 * 获取微信二维码
	 * @param seconds 有效时长，单位秒
	 * @param qrType 类型，二维码类型，QR_SCENE为临时的整型参数值，QR_STR_SCENE为临时的字符串参数值，QR_LIMIT_SCENE为永久的整型参数值，QR_LIMIT_STR_SCENE为永久的字符串参数值
	 * @param sceneStr 参数
	 * @return
	 */
	public String getQr(Long seconds, String qrType, String sceneStr) {
		String postUrl = "https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token="+accessTokenTools.getAccessToken();
		String paramXml = "{\"expire_seconds\": "+seconds+", \"action_name\": \""+qrType+"\", \"action_info\": {\"scene\": {\"scene_str\": \""+sceneStr+"\"}}}";
		JSONObject jsonObj = WeixinUtil.httpRequest(postUrl, "POST", paramXml);
		String res = jsonObj==null?"":jsonObj.toString();
		return res;
	}
